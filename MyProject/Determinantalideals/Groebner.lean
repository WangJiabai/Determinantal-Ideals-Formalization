...
/--
Lemma 5, leading-exponent version.

For the concrete `antiDiagonalLex` implementation, the leading exponent of a
minor is its anti-diagonal exponent. Since `MonomialOrder.degree` records the
exponent vector of the leading monomial, this is the coefficient-free layer of
the paper’s `init(D)` statement.
-/
...
/-- Lemma 5 correspondence alias for the leading-exponent statement. -/
...
/-- Lemma 5 correspondence alias for the coefficient-sensitive leading term. -/
...
/--
The Gröbner-basis component of Sturmfels’s Theorem 1, in a
term-order-parametric form.

For every monomial order satisfying `IsAntidiagonalTermOrder`, the set of all
`(r + 1) × (r + 1)` generic minors is a Gröbner basis of the ideal `Jr`
generated by those minors.

Compared with the source statement, this theorem is characteristic-free over
fields and abstracts over the anti-diagonal term order. It does not formalize
the geometric rank-variety interpretation or the reducedness conclusion. The
latter would require coefficient normalization and a separate proof that every
non-leading monomial of each normalized minor is reduced with respect to the
leading monomials of the other generators.
-/
theorem theorem1_GrPlusOne_isGroebnerBasis_of_isAntidiagonalTermOrder
  ...
  -- Lemma 5 identifies the leading exponent of every generator in `G_{r+1}`.
  have hLemma5 :
      ∀ I : MinorIndex m n (r + 1),
        ord.degree (genericMinor k I) = antidiagExp I := by
    intro I
    exact degree_minor_eq_antidiagExp k ord hanti I
  -- Lemma 6 gives the width criterion for the coefficient-one anti-diagonal
  -- monomial ideal. The coefficient-sensitive connection is established below.
  have hLemma6 :
      ∀ E : (Fin m × Fin n) →₀ ℕ,
        MvPolynomial.monomial E (1 : k) ∈ initGrPlusOne m n r k
          ↔ r + 1 ≤ monomialWidth E := by
  ...
  have hnot_initial_subset :
    ...
    exact
      (MonomialOrder.IsGroebnerBasis.isGroebnerBasis_iff
        ...
        ⟨hG_subset_J, hsubset⟩
  -- First use the exponent theorem to identify the anti-diagonal exponent.
  -- The leading coefficient is the reverse-permutation sign, hence a unit over
  -- the field. Thus the signed leading terms generate the same ideal as the
  -- corresponding coefficient-one anti-diagonal monomials.
...
/--
A term-order-parametric version of the Gröbner-basis component of
Sturmfels’s Theorem 1.
...
/--
The concrete `antiDiagonalLex` specialization of the Gröbner-basis component
of Sturmfels’s Theorem 1.
