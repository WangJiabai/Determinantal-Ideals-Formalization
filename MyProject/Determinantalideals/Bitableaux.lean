...
/-- The determinantal ideal `J_r` of the paper.

It is generated by all `(r + 1) × (r + 1)` minors of the generic `m × n` matrix over `k`. -/
abbrev Jr (m n r : ℕ) (k : Type*) [CommRing k] :=
  detIdeal m n (r + 1) k

/-- The set `G_{r+1}` of all `(r + 1) × (r + 1)` generic minors.

This is the candidate Gröbner basis for `J_r` in Sturmfels' Theorem 1. -/
