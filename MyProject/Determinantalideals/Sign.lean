...
noncomputable def eps (α : Type*) [CommRing α] {t : ℕ}
    (σ : Equiv.Perm (Fin t)) : α :=
  (((Equiv.Perm.sign σ : ℤˣ) : ℤ) : α)

@[simp] lemma eps_one :
    eps α (1 : Equiv.Perm (Fin t)) = 1 := by
  simp [eps]

@[simp] lemma eps_mul_eps (σ τ : Equiv.Perm (Fin t)) :
    eps α (σ * τ) = eps α σ * eps α τ := by
  simp [eps]
...
lemma det_submatrix_rows_perm (σ : Equiv.Perm (Fin t)) :
    (A.submatrix σ id).det = eps α σ * A.det := by
...
lemma det_submatrix_cols_perm (σ : Equiv.Perm (Fin t)) :
    (A.submatrix id σ).det = eps α σ * A.det := by
  simpa [eps] using (Matrix.det_permute' A (σ := σ))
...
lemma det_submatrix_rows_cols_perm (σ τ : Equiv.Perm (Fin t)) :
    (A.submatrix σ τ).det = eps α σ * eps α τ * A.det := by
  calc
    (A.submatrix σ τ).det
        = ((A.submatrix σ id).submatrix id τ).det := rfl
    _ = eps α τ * (A.submatrix σ id).det := by
        simpa using
          (det_submatrix_cols_perm
            (A := A.submatrix σ id) τ)
    _ = eps α τ * (eps α σ * A.det) := by
        rw [det_submatrix_rows_perm σ]
    _ = eps α σ * eps α τ * A.det := by
...
noncomputable def minorMatrix (k : Type*) [CommRing k] {m n t : ℕ}
    (I : MinorIndex m n t) :
...
@[simp] lemma minor_eq_det_minorMatrix (I : MinorIndex m n t) :
    genericMinor k I = (minorMatrix k I).det := by
  rfl
