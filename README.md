...
Sturmfels’s theorem for determinantal ideals and its coefficient-normalized
reduced refinement. For a generic `m × n` matrix over an arbitrary field, it
proves that all `(r + 1) × (r + 1)` generic minors form a Gröbner basis of the
ideal they generate for every monomial order satisfying
`IsAntidiagonalTermOrder`. These original minors already satisfy the
interreduction condition. After each minor is multiplied by the inverse of its
anti-diagonal leading coefficient, the normalized minors form a reduced
Gröbner basis in the monic sense used by the imported Gröbner library. The
order `antiDiagonalLex` is a concrete instance.
...
The reducedness result is stated for the explicitly coefficient-normalized
generators, rather than identifying the unnormalized signed minors with a
monic family.
...
degree-wise Hilbert-function argument. A final unit-rescaling and divisibility
rigidity argument proves reducedness for the normalized generators. The final
theorems use anti-diagonal term orders.
...
at the final theorem and reducedness layers.
